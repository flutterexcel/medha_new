...
// this is a function to call API
  ...
  Future apicall() async {
    ...
    if (response.statusCode == 200) {
      setState(() {
        print("hgyft${response.body}");
        print("hgyft${response.body}");
        // stringResponse = response.body;
        mapResponse = json.decode(response.body);
        dataaresponse = mapResponse['data'];
      });
    }
  }
        ...
        child: Container(
            ...
            child: ListTile(
              leading: CircleAvatar(
                  backgroundImage: NetworkImage(dataaresponse['avatar'])),
              title: Text(dataaresponse['first_name'].toString()),
              subtitle: Text(dataaresponse['last_name'].toString()),
              trailing: Text(dataaresponse['email']),
            )),
      ),
      // Text(mapResponse['data'].toString()),  // all value from data
      // Text(dataaresponse['first_name'].toString()) //janet
      //-------------------------------main app-------------------------------
      // appBar: AppBar(
